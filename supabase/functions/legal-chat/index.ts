...
import { runLegalPipeline, type LegalPipelineDeps, type LegalPipelineInput } from "../_shared/legal-pipeline-orchestrator.ts";
// Phase 8.1: convert dynamic imports to static so the Supabase CLI bundler
// reliably includes these shared modules in the deployed bundle. Without
// static imports, the deployed function failed with
// "Module not found: .../legal-pipeline-orchestrator.ts".
import { checkRateLimits } from "../_shared/rate-limiter.ts";
import { callStreamBypass, type BypassResult } from "../_shared/gateway-bypass.ts";
import { getModelConfig as _getModelConfig } from "../_shared/openai-router.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    // === RATE LIMITING (P0) ===
    // Phase 8.1: checkRateLimits is now imported statically at top of file.
    ...
    // Orchestrator is now imported statically at top of file (runLegalPipeline).
    // Static import ensures the Supabase deploy bundler includes legal-pipeline-orchestrator.ts.
    ...
    // Phase 8.1: callStreamBypass and _getModelConfig are now imported statically
    // at the top of the file so the Supabase CLI bundler reliably includes
    // gateway-bypass.ts and openai-router.ts in the deployed bundle.
    ...
    const { readable, writable } = new TransformStream({
      ...
      async flush(controller) {
        ...
        try {
          // Phase 8.1: runFinalLegalQA is now imported statically at top of file.
          if (streamMode === "safe") {
          ...
          const pipelineMetadata = {
            ...
            // Phase 8.1: surface provider fallback metadata to the client so
            // it can warn the user the answer was generated by a fallback model.
            ...(streamResult.provider_fallback_meta
              ? { provider_fallback_meta: streamResult.provider_fallback_meta }
              : {}),
          };
